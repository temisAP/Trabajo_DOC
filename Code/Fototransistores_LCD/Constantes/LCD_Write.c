 ...
 *  ------------------------------------------------------------------------- 
 *
 * Created: Wed Jan 19 15:56:10 2022
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME LCD_Write
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
/* %%%-SFUNWIZ_defines_Changes_BEGIN --- EDIT HERE TO _END */
#define NUM_INPUTS            2
/* Input Port  0 */
#define IN_PORT_0_NAME        H_Angle
...
{
    const real_T *H_Angle = (real_T *) ssGetInputPortRealSignal(S, 0);
    const real_T *V_Angle = (real_T *) ssGetInputPortRealSignal(S, 1);
    const real_T *xD = ssGetDiscStates(S);

    LCD_Write_Outputs_wrapper(H_Angle, V_Angle, xD);
...
static void mdlUpdate(SimStruct *S, int_T tid)
{
    ...
    LCD_Write_Update_wrapper(H_Angle, V_Angle, xD);
